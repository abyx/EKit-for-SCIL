...
public class SimpleInfoDialog extends JDialog
{
	...
	public SimpleInfoDialog(Frame parent, String title, boolean bModal, String message, int type)
	{
		...
		if(type == QUESTION)
		{
			...
			jOptionPane = new JOptionPane(label(message), JOptionPane.PLAIN_MESSAGE, JOptionPane.OK_CANCEL_OPTION, null, buttonLabels, buttonLabels[0]);
		}
		else
		{
			buttonLabels = new Object[]{ Translatrix.getTranslationString("DialogClose") };
			jOptionPane = new JOptionPane(label(message), JOptionPane.PLAIN_MESSAGE, JOptionPane.DEFAULT_OPTION, null, buttonLabels, buttonLabels[0]);
		}
   ...
   private JLabel label(String message) {
